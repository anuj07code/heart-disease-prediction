...
class User(UserMixin, db.Model):
    ...
    # Added features for Caregiver, Gamification & Multilanguage
    caregiver_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True)
    heart_points = db.Column(db.Integer, default=0)
    ...
    # Caregiver self-reference
    dependents = db.relationship('User', backref=db.backref('caregiver', remote_side=[id]))
...
class Report(db.Model):
    ...
    # New Lifestyle & Genetic Features
    tobacco = db.Column(db.Integer, nullable=True) # 0=No, 1=Past, 2=Current
    obesity = db.Column(db.Integer, nullable=True) # 0=No, 1=Yes
    exercise_freq = db.Column(db.Integer, nullable=True) # e.g., days per week
    unhealthy_diet = db.Column(db.Integer, nullable=True) # 0=No, 1=Yes
    stress_level = db.Column(db.Integer, nullable=True) # 1-10
    genetics = db.Column(db.Integer, nullable=True) # 0=No, 1=Family History
    bmi = db.Column(db.Float, nullable=True)

    # ML & Custom AI Results
    prediction_result = db.Column(db.Integer) # 1 = High Risk, 0 = Low Risk
    probability = db.Column(db.Float)
    ai_health_tips = db.Column(db.Text, nullable=True)
