...
from gemini_integration import generate_health_tips, handle_chat_message
import qrcode
...
def ensure_patient_qrcode(user):
    """Create a patient QR code file if it is missing."""
    if user.role != 'patient':
        return

    qr_dir = os.path.join(FRONTEND_DIR, 'static', 'qrcodes')
    os.makedirs(qr_dir, exist_ok=True)
    qr_path = os.path.join(qr_dir, f"patient_{user.id}.png")
...
def dashboard():
    if current_user.role == 'patient':
        ...
        dependents_data = []
        if current_user.dependents:
            for dep in current_user.dependents:
                latest_report = Report.query.filter_by(patient_id=dep.id, status='Verified').order_by(Report.created_at.desc()).first()
                if not latest_report:
                    latest_report = Report.query.filter_by(patient_id=dep.id).order_by(Report.created_at.desc()).first()
                dependents_data.append({'user': dep, 'latest_report': latest_report})
        return render_template('patient_dashboard.html', reports=reports, dependents_data=dependents_data)
...
def account():
    ...
    if request.method == 'POST':
        ...
        elif action == 'link_caregiver':
            dependent_email = request.form.get('dependent_email', '').strip()
            dependent = User.query.filter_by(email=dependent_email).first()
            if not dependent:
                flash("We couldn't find a user with that email address.", "error")
            elif dependent.id == current_user.id:
                flash("You cannot link yourself.", "error")
            else:
                dependent.caregiver_id = current_user.id
                db.session.commit()
                flash(f"Successfully linked {dependent.name} to your Family Dashboard.", "success")
    ...
    if current_user.role == 'patient':
        ...
        ensure_patient_qrcode(current_user)
    elif current_user.role == 'doctor':
...
def predict():
    ...
    if request.method == 'POST':
        try:
            ...
            data['tobacco'] = int(request.form.get('tobacco', 0))
            data['obesity'] = int(request.form.get('obesity', 0))
            data['exercise_freq'] = int(request.form.get('exercise_freq', 0))
            data['unhealthy_diet'] = int(request.form.get('unhealthy_diet', 0))
            data['stress_level'] = int(request.form.get('stress_level', 5))
            data['genetics'] = int(request.form.get('genetics', 0))
            data['bmi'] = float(request.form.get('bmi', 22.0) or 22.0)

            ml_result = predict_heart_disease(data)
            
            # Simple heuristic modifier for Lifestyle Risk
            risk_penalty = 0.0
            if data['tobacco'] == 2: risk_penalty += 0.05
            if data['obesity'] == 1: risk_penalty += 0.03
            if data['unhealthy_diet'] == 1: risk_penalty += 0.02
            if data['genetics'] == 1: risk_penalty += 0.05
            if data['exercise_freq'] < 2: risk_penalty += 0.03
            
            final_probability = min(ml_result['probability'] + risk_penalty, 0.99)
            final_prediction = 1 if final_probability >= 0.5 else 0

            # Generate AI Health Tips
            health_tips_data = data.copy()
            health_tips_data['risk_level'] = "High Risk" if final_prediction == 1 else "Low Risk"
            health_tips_data['probability'] = final_probability
            health_tips = generate_health_tips(health_tips_data)
            ...
            new_report = Report(
                ...
                tobacco=data['tobacco'], obesity=data['obesity'], exercise_freq=data['exercise_freq'],
                unhealthy_diet=data['unhealthy_diet'], stress_level=data['stress_level'], genetics=data['genetics'], bmi=data['bmi'],
                prediction_result=final_prediction,
                probability=final_probability,
                ai_health_tips=health_tips,
                status='Pending'
            )
            db.session.add(new_report)
            
            # Gamification: Give user points for submitting health assessment
            if current_user.heart_points is None: current_user.heart_points = 0
            current_user.heart_points += 50
...
@app.route('/verify_report/<int:report_id>', methods=['GET', 'POST'])
@login_required
def verify_report(report_id):
    ...
    # Process Doctor's Note if submitted
    if request.method == 'POST':
        note = request.form.get('doctor_note', '').strip()
        if note:
            report.doctor_note = note
...
# ═══════════════════════════════════════════════════════════════
#  AI CHATBOT & GAMIFICATION ROUTES
# ═══════════════════════════════════════════════════════════════

@app.route('/api/chat', methods=['POST'])
@login_required
def chat_api():
    """Endpoint for 24/7 AI Health Chatbot interaction."""
    data = request.get_json()
    message = data.get('message', '')
    if not message:
        return jsonify({'response': 'Please provide a message.'}), 400
    
    # Simply forwarding the message; advanced feature would keep history
    bot_response = handle_chat_message(message)
    return jsonify({'response': bot_response})

@app.route('/api/gamification/steps', methods=['POST'])
@login_required
def log_steps():
    """Endpoint for gamification points when a user walks 5000+ steps."""
    if current_user.role != 'patient':
    ...
    if steps >= 5000:
        points_awarded = 100
        current_user.heart_points += points_awarded
        db.session.commit()
        return jsonify({'success': True, 'points': current_user.heart_points, 'awarded': points_awarded, 'message': 'Goal reached! +100 Heart Points!'})
        
    return jsonify({'success': True, 'points': current_user.heart_points, 'awarded': 0, 'message': 'Keep going!'})
...
@login_required
def api_chat():
    from gemini_integration import handle_chat_message
    ...
    try:
        ...
        return jsonify({'response': 'Sorry, I encountered an error connecting to my AI core.'}), 500
